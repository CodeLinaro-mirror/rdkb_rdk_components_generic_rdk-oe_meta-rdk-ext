...
inherit pkgconfig gobject-introspection systemd
...
RRECOMMENDS_${PN} += "pipewire ${PN}-scripts ${PN}-modules-meta"

FILES_${PN} += "${systemd_user_unitdir} ${systemd_system_unitdir} ${datadir}/zsh"

FILES_libwireplumber = " \
    ${libdir}/libwireplumber-*.so.* \
"

FILES_${PN}-scripts += "${datadir}/wireplumber/scripts/*"
